...
public class Output(CliOptions options) : IOutput
{
    ...
    private int GetTopPositionFromInitial(int initial, int position, int textLength)
    {
        var whichLine = (initial + position) / Console.BufferWidth;
        var lineCount = (initial + textLength) / Console.BufferWidth;
