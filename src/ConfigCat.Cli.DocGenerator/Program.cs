...
class Program
{
    ...
    static async Task Main()
    {
        ...
        output.AppendLine($"# ConfigCat CLI Reference Documentation");
        output.AppendLine("[GitHub](https://github.com/configcat/cli) | [Documentation](https://configcat.com/docs/advanced/cli) | [ConfigCat](https://configcat.com)");
        output.AppendLine("");
        output.AppendLine("The ConfigCat CLI allows you to interact with the ConfigCat Management API via the command line. It supports most functionality found on the ConfigCat Dashboard. You can manage ConfigCat resources like Feature Flags, Targeting / Percentage rules, Products, Configs, Environments, and more.");
    ...
    static async Task GenerateDocsForSubCommand(Command command, Stack<Command> parents, IDictionary<string, string> generatedDocs)
    {
        ...
        if (options.Any())
        {
            ...
            output.AppendLine("| ------ | ----------- |");
            foreach (var option in options)
                output.AppendLine($"| {string.Join(", ", option.Aliases.Select(a => $"`{a}`"))} | {option.Description.Replace(Environment.NewLine, "<br/>")}{(option.GetSuggestions().Any() && !option.GetSuggestions().Contains("True") ? $"<br/><br/>*Possible values*: {string.Join(", ", option.GetSuggestions().Select(s => $"`{s}`"))}" : "")} |");
        }
