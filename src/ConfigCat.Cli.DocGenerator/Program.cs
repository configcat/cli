...
class Program
{
    ...
    static async Task GenerateDocsForSubCommand(Command command, Stack<Command> parents, IDictionary<string, string> generatedDocs)
    {
        ...
        if (options.Any())
        {
            ...
            output.AppendLine("| ------ | ----------- |");
            foreach (var option in options)
                output.AppendLine($"| {string.Join(", ", option.Aliases.Select(a => $"`{a}`"))}{(option.GetSuggestions().Any() && !option.GetSuggestions().Contains("True") ? $" ({string.Join(",", option.GetSuggestions().Select(s => $"`{s}`"))})" : "")} | {option.Description.Replace(Environment.NewLine, "<br/>")} |");
        }
