...
namespace ConfigCat.Cli
{
    class Program
    {
        static async Task<int> Main(string[] args)
        {
            ...
            container.Register<ConfigurationHandler>(c => c.AsImplementedTypes());
            ...
            var parser = new CommandLineBuilder(root)
                ...
                {
                    var hasVerboseOption = context.ParseResult.FindResultFor(Constants.VerboseOption) != null;
                    container.RegisterInstance(new ExecutionContext { Output = new Output(context.Console, hasVerboseOption) });
                    await next(context);
                })
                .UseVersionOption()
                .UseHelp()
        ...
        private static void ExceptionHandler(Exception exception, InvocationContext context)
        {
            ...
            if (exception is OperationCanceledException || exception is TaskCanceledException)
                context.Console.WriteError("Terminated.");
            else if (exception is HttpStatusException statusException)
                context.Console.WriteError($"Http request failed: {(int)statusException.StatusCode} {statusException.ReasonPhrase}.");
            else if (exception is MaxRetryAttemptsReachedException retryException)
            {
                if (retryException.OperationResult is HttpResponseMessage response)
                    context.Console.WriteError($"Http request failed: {(int)response.StatusCode} {response.ReasonPhrase}.");
                else if(retryException.InnerException != null)
                    context.Console.WriteError(hasVerboseOption ? retryException.InnerException.ToString() : retryException.InnerException.Message);
                else
                    context.Console.WriteError(hasVerboseOption ? retryException.ToString() : retryException.Message);
            }
            else
                context.Console.WriteError(hasVerboseOption ? exception.ToString() : exception.Message);
